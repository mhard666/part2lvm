...
# part2lvmKR.sh
# v. 1.0.0  - 20210115. - mh    - part2lvm keep root - hält die root-Partition auf der ursprünglichen Platte/Part.   
...
lvmLogicalVolumeData='lv_swap 16G swap
lv_home 20G ext4 /home /mnt/dst/home
lv_opt 2G ext4 /opt /mnt/dst/opt
lv_var 10G ext4 /var /mnt/dst/var
...
#zum testen...
#lvmLogicalVolumeData='lv_swap 1G swap
...
### Loop 2.1 - erfolgreich kopierte Verzeichnisse am Ursprung löschen

log "regular" "INFO" "START Loop2.1 ==================================================================================="
while read -r line 
do
    log "regular" "DEBUG" "-------------------------- loop:"
    ...
    then
        ...
        if [ -d ${fsTgtPath%/*} ]; then
            # Dateien vom source löschen
            log "regular" "DEBUG" "pathEmptyOrDelContent $fsTgtPath"
            pathEmptyOrDelContent "$fsTgtPath"
            result=$?
            if [ $result -eq 0 ]; then
                # Rückgabewert 0 - Ok
                log "regular" "DEBUG" "fsTgtPath leer oder Inhalt gelöscht: $fsTgtPath"
                echo "0"
            elif [ $result -eq $rWARNING_PathNotEmpty ]; then
                # Rückgabewert 1 - Fehler, Verzeichnis nicht leer - Abfrage Abbruch/Weiter
                log "regular" "WARN" "fsTgtPath nicht leer und Inhalt konnte nicht gelöscht werden: $fsTgtPath"
                echo "$result - Path not empty"
                ### ToDo: Abfrage bei Fehler ob weiter oder Abbruch
            else
                # Rückgabewert 2 oder höher - Fehler bei Parameterübergabe - Abbruch
                log "regular" "ERROR" "fsTgtPath - Fehler beim löschen. Abbruch!"
                echo "$result - Abbruch"
                exit $result
            fi
        fi
